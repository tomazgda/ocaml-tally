...
let summarise directory =
  let get_movements = List.map ~f:(fun t -> t.movements) >> List.concat in

  let add_amount (db1, cr1) (db2, cr2) = (db1 + db2, cr1 + cr2) in
  ...
  let sum_movements movements =
    ...
    List.fold movements ~init:empty_accounts ~f
  in

  let joint_bal, expense_bal, income_bal =
    directory |> Reader.parse_directory |> get_movements |> sum_movements
  in
